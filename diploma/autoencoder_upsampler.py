...
class AutoencoderUpsampler(nn.Module):
    def __init__(self, in_channels=3, hidden_layers=64):
        ...
        self.res_1 = ResBlock(hidden_layers, hidden_layers)
        self.res_2 = ResBlock(hidden_layers, hidden_layers*2)
        self.res_3 = ResBlock(hidden_layers*2, hidden_layers*4)
        self.res_4 = ResBlock(hidden_layers*4, hidden_layers*4)
    ...
    def forward(self, x):
        encoded = self.start_conv(x)
        encoded = self.res_1(encoded)
        downsample_1 = self.max_pool(self.res_2(encoded))
        downsample_2 = self.max_pool(self.res_3(downsample_1))
        downsample_2 = self.res_4(downsample_2)
        upsample = self.upsample_1(downsample_2)
        upsample = upsample + downsample_1
        ...
        upsample = self.activation(self.final_conv(upsample))
        return upsample
